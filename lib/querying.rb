...
def select_series_title_with_most_human_characters
  "SELECT series.title FROM series, characters GROUP BY series.title ORDER BY COUNT(characters.species) DESC LIMIT 1"
end

def select_character_names_and_number_of_books_they_are_in
  "SELECT characters.name, COUNT(books.id) FROM books, characters, character_books WHERE characters.id = character_books.character_id AND character_books.book_id = books.id GROUP BY characters.name ORDER BY COUNT(books.id) DESC"
end
